...
- **Learning modules**: `index.html` adds Learning Paths (`#paths` + shared detail view `#path`), the Labs hub (`#labs`) linking to the Red Team (`#redteam`), Cryptography (`#cryptolab`), IR (`#ir`) and CTF (`#games`) labs, plus a Progress hub (`#progress`) and About (`#about`) — all wired in `script.js` (MODULE 36 theme switch, MODULE 37 paths/progress/about, MODULE 39 structured learning paths, plus the Red Team · IR · Crypto · tools block).
